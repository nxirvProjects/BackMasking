...
public class ListStack implements BKStack, Iterable<Double> {
    ...
    public int count() {
        // This function should use a enhanced for loop to count how many elements are
        // in the stack
        int c = 0;
    ...
    // Push function takes "double d" argument and pushes this element to the top of
    // the stack
    public void push(double d) {
        // Create temp node to help in holding element get passed through the function
        // push(double d)
        ListStackNode temp = new ListStackNode();

        temp.data = d; // holds the element data
        temp.addr = top; // holds the elements address
        top = temp; // top will change to temp's object attributes like the address and the data of
    ...
    public double pop() {
        ...
        top = top.addr; // This should store the next address of the element after popping the current
                        // element.

        modCount++; // Everytime a pop method is called this will increment because a modification
                    // was done.
    ...
    private class stackIt implements Iterator<Double> {
        ...
        int finalModCount = modCount; // Tracks the expected modifications made through calling the pop and push
                                      // methods of the stack.

        public boolean hasNext() {

            // All this means is if the modCount before iterating through the stack does not
            // equal to it after iterating through the stack it should throw a exception.
            if (finalModCount != modCount) {
                throw new ConcurrentModificationException();
            }

            if (current != null) { // If the current element is null that means there should not be a next element
                                   // in the list.
        ...
        public Double next() {
            if (!hasNext()) {
                throw new EmptyStackException(); // basically all this says is that if there isn't a next element then
